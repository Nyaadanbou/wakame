...
### 移除原版盔甲机制
移除了原版盔甲值/盔甲韧性/魔咒效果组件 `armor_effectiveness` 的伤害减免
...
### 移除女巫特权
移除了女巫会置零伤害源为自身的魔法伤害、会将带有 witch_resistant_to 标签的伤害乘以15%的特性

原因: Koish 有专门的元素机制; 而该机制基于原版伤害类型, 思路不合且移除该特性无伤大雅

### 移除安全帽
移除了原版带有 damages_helmet 标签的伤害会被头盔减免25%, 并双倍消耗头盔耐久的特性

原因: Koish 有专门的元素机制; 而该机制基于原版伤害类型, 思路不合且移除该特性无伤大雅

### 移除加倍冻伤
移除了原版带有 freeze_hurts_extra_types 标签的实体类型收到的冰冻伤害乘以5的特性
...
### 修改格挡机制
玩家举盾格挡时, blocking_damage_reduction 属性会生效, 按元素减少伤害, 单种元素伤害最低减少到0
...
可通过配置文件指定不使盔甲损失耐久的伤害类型, 可实现溺水/挤压/着火等具有防御穿透的伤害不扣除盔甲耐久(配合damage_type_mappings.yml)

可通过配置文件指定耐久损失公式, 其中伤害值为不考虑抗性提升伤害减免下的伤害(原版如此), 若产生负值会强制置零
