...
public interface KoishIngredient {
    ...
    static Ingredient ofKeys(Set<Key> keys) {
        ...
        for (Key identifier : keys) {
            ...
            KoishItemBridge.Impl.onlyCompareTypeIdForRecipeBook(mojangStack, true);
    ...
    private static @NonNull ItemStack getMojangStackByKey(Key key) {
        ServerItemRef itemRef = KoishItemBridge.Impl.getItemRef(key);
        if (itemRef != null) {
            return CraftItemStack.unwrap(itemRef.createStack(1, null));
        } else {
