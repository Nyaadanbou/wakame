...
class MutableSetCodec<A>(private val elementCodec: Codec<A>) : Codec<MutableSet<A>> {
    override fun <T> encode(input: MutableSet<A>, ops: DynamicOps<T>, prefix: T): DataResult<T> {
    ...
    override fun <T> decode(ops: DynamicOps<T>, input: T): DataResult<Pair<MutableSet<A>, T>> {
