...
public class KoishLootItem extends LootPoolSingletonContainer {
    ...
    public void createItemStack(Consumer<ItemStack> lootConsumer, LootContext context) {
        ...
        var itemRef = KoishItemBridge.Impl.getItemRef(Key.key(namespace, path));
        if (itemRef == null) {
        ...
        try {
            var bukkitStack = itemRef.createStack(1, player);
            var mojangStack = CraftItemStack.unwrap(bukkitStack);
            lootConsumer.accept(mojangStack);
        } catch (Exception e) {
