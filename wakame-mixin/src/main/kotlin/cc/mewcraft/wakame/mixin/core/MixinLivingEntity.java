...
public abstract class MixinLivingEntity {
    ...
    private float redirectComputeAmountDuringInvulnerable(LivingEntity instance, EntityDamageEvent event) {
        return DamageManagerApi.Companion.getInstance().injectDamageLogic(event, instance.lastHurt, true);
    }
    ...
    private float redirectComputeAmountDuringNotInvulnerable(LivingEntity instance, EntityDamageEvent event) {
        return DamageManagerApi.Companion.getInstance().injectDamageLogic(event, instance.lastHurt, false);
    }
    ...
    private boolean redirectIsBypassesArmor(DamageSource instance, TagKey<DamageType> damageTypeKey) {
        return DamageManagerApi.Companion.getInstance().bypassesHurtEquipment(CraftDamageType.minecraftHolderToBukkit(instance.typeHolder()));
    }
    ...
    private float redirectEquipmentDamageCompute(float a, float b) {
        // a = 1.0f
        // b = damageAmount / 4.0f
        return DamageManagerApi.Companion.getInstance().computeEquipmentHurtAmount(b * 4.0f);
    }
