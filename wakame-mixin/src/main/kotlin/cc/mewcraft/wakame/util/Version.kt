...
class Version : Comparable<Version> {
    ...
    constructor(version: String) {
        ...
        if (stage != null) {
            this.stageVersion = buildList<Int> {
                this += RELEASE_STAGES[stage]
                    ?: throw IllegalArgumentException("Unknown release stage: $stage")
                if (stageVer != null) {
                    this += stageVer.split('.').map { it.toIntOrNull() ?: 0 }
                }
            }.toIntArray()
        } else {
            this.stageVersion = intArrayOf()
    ...
    override fun toString(): String {
        return toString(omitZeros = false)
    }
    ...
    fun compareTo(other: Version, ignoreIdx: Int): Int {
        ...
        if (compare != 0 || ignoreIdx != -1) {
            // only compare stage version if an exact comparison (ignoreIdx = -1) was requested
            return compare
        }
    ...
    fun equals(other: Version, ignoreIdx: Int): Boolean {
        return compareTo(other, ignoreIdx) == 0
    }
    ...
    companion object {
        ...
        init {
            ...
            SERVER_VERSION = if (KoishSharedConstants.isRunningInIde) {
                Version(1, 21, 3)
            } else {
