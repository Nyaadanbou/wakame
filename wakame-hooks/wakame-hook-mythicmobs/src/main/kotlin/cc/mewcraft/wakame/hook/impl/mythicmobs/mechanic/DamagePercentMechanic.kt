...
class DamagePercentMechanic(
    ...
    private val damageBundle: List<String> = mlc.getStringList(arrayOf("bundle", "b"), "")
    ...
    private fun createDamageBundle(data: SkillMetadata, target: AbstractEntity): DamageBundle {
        return DamageBundleFactory.INSTANCE.createUnsafe(
            damageBundle.associate { rawPacketString ->
                ...
                when (split.size) {
                    ...
                    2 -> split[0] to DamagePacket(
                        split[0],
                        calculateDamageValue(split[1], data, target),
                        calculateDamageValue(split[1], data, target)
                    )
                    ...
                    3 -> split[0] to DamagePacket(
                        split[0],
                        calculateDamageValue(split[1], data, target),
                        calculateDamageValue(split[2], data, target)
                    )
    ...
    override fun castAtEntity(data: SkillMetadata, target: AbstractEntity): SkillResult {
        ...
        val damageMetadata = DamageMetadata(createDamageBundle(data, target), csm)
