...
internal class TownEnhancementsMenu(
    ...
    private val primaryGui: PagedGui<Item> = PagedGui.items { builder ->
        ...
        val enhancements = town.koishify()[TownHall].enhancements
    ...
    inner class EnhancementItem(
        private val enhancement: EnhancementType,
    ) : AbstractItem() {
        override fun getItemProvider(): ItemProvider {
            return when (enhancement) {
                EnhancementType.BUFF_FURNACE -> ItemWrapper(ItemStack.of(Material.FURNACE).also { it.amount = enhancement.level })
            }
        }
