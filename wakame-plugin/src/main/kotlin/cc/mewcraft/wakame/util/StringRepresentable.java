...
public interface StringRepresentable {
    ...
    static <E extends Enum<E> & StringRepresentable> StringRepresentable.EnumCodec<E> fromEnumWithMapping(
        ...
        Function<String, E> function = createNameLookup(enums, _enum -> keyFunction.apply(_enum.serializedName()));
    ...
    static <T extends StringRepresentable> Function<String, @Nullable T> createNameLookup(T[] values) {
        return createNameLookup(values, StringRepresentable::serializedName);
    }

    static <T> Function<String, @Nullable T> createNameLookup(T[] values, Function<T, String> serializer) {
        if (values.length > 16) {
            Map<String, T> map = Arrays.stream(values).collect(Collectors.toMap(serializer, object -> (T) object));
            return map::get;
        } else {
            return string -> {
                for (T object : values) {
    ...
    static Keyable keys(final StringRepresentable[] serializables) {
        return new Keyable() {
            ...
            public <T> Stream<T> keys(DynamicOps<T> ops) {
                return Arrays.stream(serializables).map(StringRepresentable::serializedName).map(ops::createString);
            }
        };
    }

    static class EnumCodec<E extends Enum<E> & StringRepresentable> extends StringRepresentable.StringRepresentableCodec<E> {
    ...
    static class StringRepresentableCodec<S extends StringRepresentable> implements Codec<S> {
        ...
        public StringRepresentableCodec(S[] values, Function<String, @Nullable S> nameLookup, ToIntFunction<S> indexLookup) {
            this.codec = ExtraCodecs.orCompressed(
                    Codec.stringResolver(StringRepresentable::serializedName, nameLookup),
