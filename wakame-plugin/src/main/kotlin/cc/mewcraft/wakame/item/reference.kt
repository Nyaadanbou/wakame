 ...
 */
interface ItemRef : ServerItemRef {
    companion object {
        /**
         * [ItemRef] 的序列化器.
         *
     ...
     * @throws cc.mewcraft.wakame.item.ItemStackGenerationException 如果物品堆叠创建失败
     */
    fun createItemStack(amount: Int = 1, player: Player? = null): ItemStack

    // 统一实现 ServerItemRef
    override fun createStack(amount: Int, player: Player?): ItemStack {
        return createItemStack(amount, player)
    }
}
