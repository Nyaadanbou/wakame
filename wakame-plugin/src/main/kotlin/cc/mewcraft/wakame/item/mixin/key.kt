...
import cc.mewcraft.wakame.KoishSharedConstants.KOISH_NAMESPACE
import cc.mewcraft.wakame.LOGGER
import cc.mewcraft.wakame.bridge.item.ServerItemKey
import cc.mewcraft.wakame.item.KoishItem
import cc.mewcraft.wakame.item.ProxyKoishItem
 ...
 */
class ItemKey private constructor(val id: KoishKey) : ServerItemKey {
    companion object {
        @JvmField
        val CODEC: Codec<ItemKey> = KoishKeys.CODEC.comapFlatMap(
            { string ->
                try {
        ...
        fun serializer(): SimpleSerializer<ItemKey> {
            return object : SimpleSerializer<ItemKey> {
    ...
    val itemType: KoishItem by when (id.namespace()) {
        KOISH_NAMESPACE -> runCatching {
            BuiltInRegistries.ITEM.createEntry(id)
        }.getOrElse {
        ...
        MINECRAFT_NAMESPACE -> runCatching {
            BuiltInRegistries.ITEM_PROXY.createEntry(id)
        }.getOrElse {
            objectPool.remove(id)
            LOGGER.error("Failed to find ${ProxyKoishItem::class.simpleName} entry with id: $id, using empty item proxy instead")
            RegistryEntry.Direct(ProxyKoishItem.EMPTY)
        }
...
internal object ItemKeyBootstrap {
    @InitFun
    fun init() {
        @Suppress("UNCHECKED_CAST") // 这个 cast 是安全的, 因为 ItemKey 是 ServerItemKey 的唯一实现
        ServerItemKey.codec = ItemKey.CODEC as Codec<ServerItemKey>
        LOGGER.info("Registered Codec for ${ServerItemKey::class.simpleName}")
    }
}