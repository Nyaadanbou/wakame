...
import cc.mewcraft.wakame.item.mixin.getDataContainer
import cc.mewcraft.wakame.item.mixin.getItemKey
...
val Material.itemProxy: ProxyKoishItem? get() = BuiltInRegistries.ITEM_PROXY[key()]
 ...
 * 获取该物品类型的套皮物品的实例.
 */
val Item.itemProxy: ProxyKoishItem?
...
val MojangStack.typeId: KoishKey
    ...
    // FIXME 临时版本, 用于过渡
    get() = getItemKey()?.id ?: getDataContainer()?.get(ItemDataTypes.ID)?.id ?: CraftItemType.minecraftToBukkit(item).key()
...
val MojangStack.koishTypeId: KoishKey?
    ...
    // FIXME 临时版本, 用于过渡
    get() = getItemKey()?.id ?: getDataContainer()?.get(ItemDataTypes.ID)?.id
...
val MojangStack.koishItem: KoishItem?
    ...
    // FIXME 临时版本, 用于过渡
    get() {
        val itemType = getItemKey()?.itemType ?: getDataContainer()?.get(ItemDataTypes.ID)?.itemType ?: item.itemProxy ?: return null
        if (itemType.isEmpty()) {
            this.count = 0
            return null
        }
...
val MojangStack.exactKoishItem: KoishItem?
    ...
    // FIXME 临时版本
    get() {
        val itemType = getItemKey()?.itemType ?: getDataContainer()?.get(ItemDataTypes.ID)?.itemType ?: return null
        if (itemType.isEmpty()) {
            this.count = 0
            return null
        }
 ...
 * 获取该物品堆叠的套皮物品的实例.
 */
val MojangStack.proxyKoishItem: ProxyKoishItem?
    get() = item.itemProxy
...
fun MojangStack.dataContainer(includeProxy: Boolean): ItemDataContainer? =
    getDataContainer() ?: if (includeProxy) item.itemProxy?.data else null
...
/**
 * 获取该物品堆叠的属性数据容器 [cc.mewcraft.wakame.item.property.ItemPropContainer].
 *
 * *绝大多数情况下无需使用该函数.*
 *
...
var MojangStack.onlyCompareTypeIdInRecipeBook: Boolean
    ...
    set(value) {
        if (value) {
            removeData(ItemDataTypes.ONLY_COMPARE_TYPE_ID_IN_RECIPE_BOOK)
        } else {
            setData(ItemDataTypes.ONLY_COMPARE_TYPE_ID_IN_RECIPE_BOOK)
        }
    }
