...
import cc.mewcraft.wakame.bridge.MojangComponent
...
object HotfixItemModel {
    ...
    fun transform(itemstack: MojangStack) {
        val itemModel = getItemModel(itemstack) ?: return
        itemstack.set(DataComponents.ITEM_MODEL, itemModel)
    }

    fun getItemModel(itemstack: ItemStack): Key? {
        val itemModel = getItemModel(itemstack.toNMS()) ?: return null
        return PaperAdventure.asAdventure(itemModel)
    }

    fun getItemModel(itemstack: MojangStack): Identifier? {
        val koishTypeId = itemstack.koishTypeId ?: return null
        ...
        if (clientboundItemModel != null) {
            ...
            return PaperAdventure.asVanilla(clientboundItemModel)
        }
        if (!itemstack.hasNonDefault(DataComponents.ITEM_MODEL)) {
            ...
            return PaperAdventure.asVanilla(koishTypeId)
        }
...
object HotfixItemName {
    ...
    fun getItemName(itemstack: ItemStack): Component? {
        val itemName = getItemName(itemstack.toNMS()) ?: return null
        return PaperAdventure.asAdventure(itemName)
    }

    fun getItemName(itemstack: MojangStack): MojangComponent? {
...
object HotfixWeaponCooldownDisplay {
     ...
     */
    fun transform(itemstack: MojangStack) {
        val useCooldown = getUseCooldown(itemstack) ?: return
        itemstack.set(DataComponents.USE_COOLDOWN, useCooldown)
    }

    fun getUseCooldown(itemstack: ItemStack): io.papermc.paper.datacomponent.item.UseCooldown? {
        val useCooldown = getUseCooldown(itemstack.toNMS()) ?: return null
        return PaperUseCooldown(useCooldown)
    }

    fun getUseCooldown(itemstack: MojangStack): UseCooldown? {
