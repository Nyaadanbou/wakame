...
object FilterFactory {
    fun create(node: ConfigurationNode): Filter {
        val type0 = node.node("type").typedRequire<String>()
        ...
        val ret: Filter = when (type) {
            ...
            "item_level" -> {
                val level = node.node("level").typedRequire<String>().let { RangeParser.parseIntRange(it) }
                ItemLevelFilter(invert, level)
            }

            "mark" -> {
                val meta = node.node("mark").typedRequire<String>()
            ...
            "rarity" -> {
                val rarity = node.node("rarity").typedRequire<Rarity>()
                RarityFilter(invert, rarity)
            }

            "core" -> {
                val coreKey = node.node("key").typedRequire<Key>()
                CoreFilter(invert, coreKey)
            }

            "toss" -> {
                val chance = node.node("chance").typedRequire<Float>()
