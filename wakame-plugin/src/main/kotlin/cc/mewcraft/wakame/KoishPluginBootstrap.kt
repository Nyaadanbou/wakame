...
import cc.mewcraft.wakame.util.Version
import cc.mewcraft.wakame.util.VersionRange
...
private val REQUIRED_SERVER_VERSION: VersionRange = Version("1.21.11")..Version("1.21.11")
internal val PREVIOUS_KOISH_VERSION: Version? = PermanentStorage.retrieveOrNull<Version>("last_version")

internal class KoishPluginBootstrap : PluginBootstrap {

    init {
        KoishBootstrapContexts.registerBootstrap(this)
    }
    ...
    override fun bootstrap(context: BootstrapContext) {
        KoishBootstrapContexts.registerLifecycleManagerOwnedByBootstrap(context.lifecycleManager)
        KoishBootstrapContexts.registerAuthors(context.pluginMeta.authors)
        KoishBootstrapContexts.registerName(context.pluginMeta.name)
        KoishBootstrapContexts.registerVersion(Version(context.pluginMeta.version))
        KoishBootstrapContexts.registerPluginJar(context.pluginSource)
        ...
        if (Version.SERVER_VERSION !in REQUIRED_SERVER_VERSION) {
            throw Exception(
                ...
                Koish v${KoishBootstrapContexts.PLUGIN_VERSION} only runs on $REQUIRED_SERVER_VERSION.
                """.trimIndent()
            )
        }
