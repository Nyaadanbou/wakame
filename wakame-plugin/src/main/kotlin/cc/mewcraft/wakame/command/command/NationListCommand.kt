...
import cc.mewcraft.wakame.gui.towny.NationListMenu
...
internal object NationListCommand : KoishCommandFactory<Source> {

    private const val COMMAND_NAME = "nationlist"

    override fun KoishCommandFactory.Builder<Source>.createCommands() {
        val builder = build(COMMAND_NAME) {
            permission(CommandPermissions.NATION_LIST)
        }

        // <root> [player]
        buildAndAdd(builder) {
            optional("player", MultiplePlayerSelectorParser.multiplePlayerSelectorParser())
            koishHandler(handler = ::handleNationList)
        }
    }

    private suspend fun handleNationList(context: CommandContext<Source>) {
        ...
        targets.forEach { target ->
            val menu = NationListMenu(target)
