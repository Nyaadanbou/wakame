...
object ItemDataFixer {
    ...
    fun needFix(rootNode: ConfigurationNode): Boolean {
        return rootNode.node(versionKey).getInt(-1) != KoishSharedConstants.ITEM_STACK_DATA_VERSION
    }

    @JvmStatic
    fun validate(rootNode: ConfigurationNode) {
        if (!KoishSharedConstants.isRunningInIde && rootNode.node(versionKey).getInt(-1) != KoishSharedConstants.ITEM_STACK_DATA_VERSION) {
            throw IllegalStateException("The data version of item $rootNode does not match the expected version ${KoishSharedConstants.ITEM_STACK_DATA_VERSION}.")
        }
    }
}
