 ...
 * 代表一个流水线.
 */
interface LorePipeline<C, I, O> {
    companion object {
        /**
         * 创建一个新的流水线.
         */
        fun <C, I, O> create(handler: PipelineHandler<C, I, O>): LorePipeline<C, I, O> {
            return SimpleLorePipeline(handler)
        }
    }
     ...
     */
    fun <K> concat(next: PipelineHandler<C, O, K>): LorePipeline<C, I, K>

    /**
     * 拼接一个新的无上下文处理器到末尾, 并返回新的流水线.
     */
    fun <K> concat(next: PipelineHandler.Stateless<O, K>): LorePipeline<Nothing, I, K>
...
private class SimpleLorePipeline<C, I, O>(
    private val handler: PipelineHandler<C, I, O>,
) : LorePipeline<C, I, O> {

    override fun <K> concat(next: PipelineHandler<C, O, K>): LorePipeline<C, I, K> {
        return SimpleLorePipeline { context: C, input: I ->
            next.process(context, handler.process(context, input))
        }
    }

    override fun <K> concat(next: PipelineHandler.Stateless<O, K>): LorePipeline<Nothing, I, K> {
        if (handler !is PipelineHandler.Stateless) {
            throw IllegalStateException("Cannot concat stateful pipeline without context.")
        }

        return SimpleLorePipeline(PipelineHandler.Stateless { input: I ->
