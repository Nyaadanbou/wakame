...
import cc.mewcraft.wakame.damage.MolangDamageMetadataBuilder
...
interface Lightning : Skill {
    ...
    val damageMetadataBuilder: MolangDamageMetadataBuilder
    ...
    companion object Factory : SkillFactory<Lightning> {
        override fun create(key: Key, config: ConfigurationNode): Lightning {
            ...
            val damageMetadata = config.node("damage_metadata").krequire<MolangDamageMetadataBuilder>()
    ...
    private class Impl(
        ...
        override val damageMetadataBuilder: MolangDamageMetadataBuilder,
    ...
    override fun tickCast(tickCount: Long): TickResult {
        ...
        for (entity in entitiesBeStruck) {
            if (entity is LivingEntity) {
                entity.hurt(skill.damageMetadataBuilder.build(), caster, false)
            }
        }
        return TickResult.ALL_DONE
    }
