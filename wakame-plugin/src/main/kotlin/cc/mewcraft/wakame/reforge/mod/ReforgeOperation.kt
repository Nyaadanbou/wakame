...
private constructor(
    ...
    private fun execute(): ModdingSession.ReforgeResult {
        ...
        val usableInput = usableInput ?: run {
            ...
            return ReforgeResult.failure(MessageConstants.MSG_MODDING_RESULT_FAILURE_FOR_NON_LEGIT_SOURCE_ITEM)
        }

        val itemRule = session.itemRule ?: run {
            logger.info("Skipped reforge as the item rule is null")
            return ReforgeResult.failure(MessageConstants.MSG_MODDING_RESULT_FAILURE_FOR_ITEM_RULE_NOT_FOUND)
        }
        ...
        if (changedReplaceParams.isEmpty()) {
            ...
            return ReforgeResult.failure(MessageConstants.MSG_MODDING_RESULT_FAILURE_FOR_ALL_CORES_NOT_APPLICABLE)
        }
        ...
        if (changedReplaceParams.any { (_, repl) -> !repl.latestResult.applicable }) {
            ...
            return ReforgeResult.failure(MessageConstants.MSG_MODDING_RESULT_FAILURE_FOR_SOME_CORE_NOT_APPLICABLE)
        }
        ...
        if (changedReplaceParams.any { (_, repl) -> (repl.usableInput?.reforgeHistory?.modCount ?: 0) >= itemRule.modLimit }) {
            ...
            return ReforgeResult.failure(MessageConstants.MSG_MODDING_RESULT_FAILURE_FOR_REACHING_MOD_COUNT_LIMIT)
        }

        // 如果源物品不合法, 则返回失败
        val builder = usableInput.cells?.builder() ?: run {
            logger.info("No cells found in source item")
            return ReforgeResult.failure(MessageConstants.MSG_MODDING_RESULT_FAILURE_FOR_NO_CELLS_FOUND_IN_SOURCE_ITEM)
        }
