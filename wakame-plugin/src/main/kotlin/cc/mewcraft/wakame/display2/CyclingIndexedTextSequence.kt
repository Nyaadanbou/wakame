 ...
 *
 * ## 开发日记 2024/7/13
 * 由于 [IndexedTextFlatter] 内部的算法限制, 如果多个 [IndexedText] 如果拥有相同的索引,
 * 那么最终只有第一个 [IndexedText] 会被渲染出来. 为了解决这个问题, 我们设计了 [CyclingIndexedTextSequence],
 * 只需要调用 [next] 就可以获得一个内容一致但索引不同的 [IndexedText]. 同时为了更高的性能,
 * [next] 返回的 [IndexedText] 实际上是从一个对象池中获取的, 并且会根据调用的顺序进行循环.
 */
internal class CyclingIndexedTextSequence(
    /**
