 ...
 * [RangedAttribute] 的字段都是原始类型, 而 [ElementAttribute] 还带有一个 [cc.mewcraft.wakame.element.Element] 的字段.
 * 也就是说, 想要创建 [ElementAttribute] 必须得先有一个 [cc.mewcraft.wakame.element.Element] 实例, 这种依赖关系使得代码变得复杂.
 * 这就直接导致了, 比如说, 客户端代码想要获取一个 [ElementAttribute], 那么它必须得先有一个 [cc.mewcraft.wakame.element.Element] 实例.
 *
 * 而在很多时候, 客户端代码并不关心 [cc.mewcraft.wakame.element.Element] 实例是什么, 它只是想要指定一个*任意元素*的 [ElementAttribute] 实例.
 * 为了解决这个问题, 我们引入了 [AttributeGetter] 这个“中间对象”.
 * 这个中间对象相当于是一个 `[cc.mewcraft.wakame.element.Element] -> [ElementAttribute]` 的函数, 而不是一个直接的 [ElementAttribute] 实例.
 *
...
interface AttributeGetter {
     ...
     * 如果传入的 [id] 无法找到对应的 [cc.mewcraft.wakame.element.Element] 实例, 则返回 `null`.
     ...
     * 对于同一个 [cc.mewcraft.wakame.element.Element] 实例, 该函数始终会返回同一个 [ElementAttribute] 实例.
     *
     * @param element 元素类型
     */
