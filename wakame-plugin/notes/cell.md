...
在这样的架构下，我们可以做出让 `sword:demo` 这个物品只能在 `iron` 重铸台下进行“初阶重铸”；但在 `gold` 重铸台下就可以进行“中阶重铸”。

当然，如果策划对于所有

每个文件里又设置了“每个物品，每个词条栏，具体的重铸参数”。

P.S.
这也意味着，我们可以创建多个不同的重铸台；每个重铸台可重铸的物品、每个物品具体的重铸效果，都可以作差异化。类似泰拉瑞亚里的合成台。
...
特点：所有内容都集中放在一个文件里。内容较少时比较好管理，但多起来就比较困难。
...
特点：极致的分门别类。内容较少时会显得有点啰嗦，但长期来看可以应付逐渐膨胀的内容。
